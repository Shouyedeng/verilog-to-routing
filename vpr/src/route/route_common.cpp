...
void print_route(FILE* fp, const vtr::vector<ClusterNetId, t_traceback>& tracebacks) {
    ...
    unsigned long chip_x = device_ctx.grid.width();
	unsigned long chip_y = device_ctx.grid.height();
    size_t nodes_per_chan = device_ctx.chan_width.max;

    vtr::NdMatrix<std::string, 4> gcr_sbox_conn({chip_x, chip_y, 4, nodes_per_chan});
    int from_track;
    int to_track = -1;
	int from_x, from_y;
	t_rr_type from_type;
        ...
        if (!cluster_ctx.clb_nlist.net_is_ignored(net_id)) {
            ...
            if (cluster_ctx.clb_nlist.net_sinks(net_id).size() == false) {
                ...
                while (tptr != nullptr) {
                    ...
                    if (rr_type == CHANY || rr_type == CHANX) {
                        if (from_track == -1) {
                            from_track = device_ctx.rr_nodes[inode].ptc_num();
                            from_x = ilow;
                            from_y = jlow;
                            from_type = rr_type;
                        } else {
                            to_track = device_ctx.rr_nodes[inode].ptc_num();

                            load_gcr_sbox_conn(gcr_sbox_conn, from_x, from_y, from_type, from_track, ilow, jlow, rr_type, to_track, size_t(net_id));
    ...
    print_gcr_sbox_echo(nodes_per_chan, gcr_sbox_conn);
}
...
void load_gcr_sbox_conn(vtr::NdMatrix<std::string, 4> &gcr_sbox_conn, 
    ...
    //VTR_LOG("gcr net id %zu\t", net_id);
    net_id = net_id + 1;
    if(from_type == CHANY && to_type == CHANY){
		if(from_x != to_x){
			printf("Error: CHANY to CHANY, but from_x != to_x, from_x = %d, to_x = %d\n", from_x, to_x);
		}
		if(to_y < from_y) { //goes down wards
			gcr_sbox_conn[to_x][to_y][TOP][from_track] = gcr_sbox_conn[to_x][to_y][TOP][from_track].empty() ? to_string(net_id) + "d" : "";
			gcr_sbox_conn[to_x][to_y][BOTTOM][to_track] = to_string(net_id) + "r";
		}else{
			gcr_sbox_conn[from_x][from_y][BOTTOM][from_track] = gcr_sbox_conn[from_x][from_y][BOTTOM][from_track].empty() ? to_string(net_id) + "d" : "";
			gcr_sbox_conn[from_x][from_y][TOP][to_track] = to_string(net_id) + "r";
		}
	}else if(from_type == CHANY && to_type == CHANX){
		if(to_x > from_x && to_y == from_y){ //goes upper right
			gcr_sbox_conn[from_x][from_y][BOTTOM][from_track] = gcr_sbox_conn[from_x][from_y][BOTTOM][from_track].empty() ? to_string(net_id) + "d" : ""; 
			gcr_sbox_conn[from_x][from_y][RIGHT][to_track] = to_string(net_id) + "r";

		}else if(to_x > from_x && to_y < from_y){ //goes lower right
			gcr_sbox_conn[from_x][to_y][TOP][from_track] = gcr_sbox_conn[from_x][to_y][TOP][from_track].empty() ? to_string(net_id) + "d" : "";
			gcr_sbox_conn[from_x][to_y][RIGHT][to_track] = to_string(net_id) + "r";

		}else if(to_x == from_x && to_y == from_y){
			gcr_sbox_conn[from_x][from_y][BOTTOM][from_track] = gcr_sbox_conn[from_x][from_y][BOTTOM][from_track].empty() ? to_string(net_id) + "d" : "";
			gcr_sbox_conn[from_x][from_y][LEFT][to_track] = to_string(net_id) + "r";
		
		}else if(to_x == from_x && to_y < from_y){
			gcr_sbox_conn[to_x][to_y][TOP][from_track] = gcr_sbox_conn[to_x][to_y][TOP][from_track].empty() ? to_string(net_id) + "d" : "";
			gcr_sbox_conn[to_x][to_y][LEFT][to_track] = to_string(net_id) + "r";
		
		}else{
			printf("Error: CHANY to CHANY in coordinates of from node and to node\n");
		}
	}else if(from_type == CHANX && to_type == CHANX){
		if(to_x > from_x){
			gcr_sbox_conn[from_x][from_y][LEFT][from_track] =  gcr_sbox_conn[from_x][from_y][LEFT][from_track].empty() ? to_string(net_id) + "d" : "";
			gcr_sbox_conn[from_x][from_y][RIGHT][to_track] = to_string(net_id) + "r"; 
		
		}else if(to_x < from_x){
			gcr_sbox_conn[to_x][to_y][RIGHT][from_track] =  gcr_sbox_conn[to_x][to_y][RIGHT][from_track].empty() ? to_string(net_id) + "d" : ""; 
			gcr_sbox_conn[to_x][to_y][LEFT][to_track] = to_string(net_id) + "r"; 
		}else{
			printf("Error: CHANX to CHANX in coordinates of from node and to node\n");
			
		}
	
	}else if(from_type == CHANX && to_type == CHANY){
		if(to_x == from_x && to_y > from_y){
			gcr_sbox_conn[from_x][from_y][LEFT][from_track] = gcr_sbox_conn[from_x][from_y][LEFT][from_track].empty() ? to_string(net_id) + "d" : "";
			gcr_sbox_conn[from_x][from_y][TOP][to_track] = to_string(net_id) + "r";
		
		}else if(to_x == from_x && to_y == from_y){
			gcr_sbox_conn[to_x][to_y][LEFT][from_track] =  gcr_sbox_conn[to_x][to_y][LEFT][from_track].empty() ? to_string(net_id) + "d": ""; 
			gcr_sbox_conn[to_x][to_y][BOTTOM][to_track] = to_string(net_id) + "r"; 
			...
			gcr_sbox_conn[to_x][from_y][RIGHT][from_track] = gcr_sbox_conn[to_x][from_y][RIGHT][from_track].empty() ? to_string(net_id) + "d" : ""; 
			gcr_sbox_conn[to_x][from_y][TOP][to_track] = to_string(net_id) + "r";
		}else{
			printf("Error: CHANX to CHANY in coordinates of from node and to node\n");
...
//to ensure that each net appear if and only if twice at one switchbox location
int check_gcr_input(vtr::NdMatrix<std::string, 4> &gcr_sbox_conn){
	...
	size_t net_id;
	for (size_t l = 0; l < chip_x; ++l) {
        for (size_t k = 0; k < chip_y; ++k) {
			map<size_t, vector<pair<size_t, size_t>>> hmap;
            
            for (size_t j = 0; j < nodes_per_chan; ++j) {
				for(size_t i = 0; i < 4; ++i){
			    	size_t len = gcr_sbox_conn[l][k][i][j].size();
					if(len == 0) continue;
					assert(len > 1);
					net_id = atoi(gcr_sbox_conn[l][k][i][j].substr(0, len - 1).c_str());	
			    	hmap[net_id].push_back(make_pair(i, j));
				}
			}
			map<size_t, vector<pair<size_t, size_t>>>::iterator it;
			for(it = hmap.begin(); it != hmap.end(); it++){
				if(it->second.size() != 2){ //net_id coccurs more than twice, then clear this net on gcr_sbox_conn
				    printf("Error: in gcr input file at location (%zu, %zu), net %zu appears %zu times\n", l, k, it->first, it->second.size());
					return -1;
				}
			}// end for
		} //end for
...
void print_gcr_sbox_echo(size_t nodes_per_chan, vtr::NdMatrix<std::string, 4> &gcr_sbox_conn){
	...
	if(-1 == check_gcr_input(gcr_sbox_conn)){
		printf("Error: in check_gcr_input\n");
		
	}

	FILE* out = vtr::fopen("gcr_sbox_conn.txt", "w");
	if(!out) {
		printf("Error: in open gcr_sbox_conn.echo file for write\n");
        exit(-1);	
	}
    fprintf(out, "* %zu\n", chip_x * chip_y);
    for (size_t l = 0; l < chip_x; ++l) {
            for (size_t k = 0; k < chip_y; ++k) {
                fprintf(out, "* %zu %zu %zu %zu ", l, k, nodes_per_chan, nodes_per_chan);
                for (size_t j = 0; j < nodes_per_chan; ++j) {
                    fprintf(out, "%s ", gcr_sbox_conn[l][k][LEFT][j].empty() ? "0" : gcr_sbox_conn[l][k][LEFT][j].c_str());
				}
                for (size_t j = 0; j < nodes_per_chan; ++j) {
                    fprintf(out, "%s ", gcr_sbox_conn[l][k][BOTTOM][j].empty() ? "0" : gcr_sbox_conn[l][k][BOTTOM][j].c_str());
                	for (size_t i = 0; i < nodes_per_chan; ++i) {
                    	fprintf(out, ". ");
					}
                    fprintf(out, "%s ", gcr_sbox_conn[l][k][TOP][j].empty() ? "0": gcr_sbox_conn[l][k][TOP][j].c_str());
					
				}
                for (size_t j = 0; j < nodes_per_chan; ++j) {
                    fprintf(out, "%s ", gcr_sbox_conn[l][k][RIGHT][j].empty() ? "0" : gcr_sbox_conn[l][k][RIGHT][j].c_str());
				}
                fprintf(out, "\n");
            }
            //fprintf(out, "\n");
